...
const setLatestMatchEpoch = (epoch) => {
  localStorage.setItem(`latestMatchEpoch-${getUserId()}`, epoch);
};

const getLatestMatchEpoch = () => {
  return parseInt(
    localStorage.getItem(`latestMatchEpoch-${getUserId()}`) || 0,
    10
  );
};
...
export const MatchesProvider = ({ children }) => {
  ...
  const processMatches = async (matches) => {
    ...
    console.log("~~ @ getLatestMatchEpoch:", getLatestMatchEpoch());

    const filteredMatches = sortedMatches.filter((match) => {
      console.log(
        "~~ @ filteredMatches match.created_at:",
        new Date(match.created_at).getTime(),
        "getLatestMatchEpoch() + 1:",
        getLatestMatchEpoch() + 1,
        "keep:",
        new Date(match.created_at).getTime() > getLatestMatchEpoch() + 1
      );
      return new Date(match.created_at).getTime() > getLatestMatchEpoch() + 1;
    });
